...
-- Lua 5.1 shims (Roblox executors that lack 5.2/Luau extras)
...
if type(genv.__TND_HUB_SHUTDOWN) == "function" then
	pcall(genv.__TND_HUB_SHUTDOWN)
	task.wait(0.1)
end
if genv.__TND_HUB_RUNNING then
	return
end
genv.__TND_HUB_RUNNING = true
...
-- Executor GUI compat:
-- Some callback threads lose the capability required to touch hidden/CoreGui instances later.
-- Force the UI to mount into PlayerGui while its Env module is initialized.
local _tndUiCompat = {
...
local _uiOk, BobloUI = pcall(function()
	return loadstring(game:HttpGet(
		"https://raw.githubusercontent.com/TND-Hub/TND-Hub/refs/heads/main/TNDHubUI.min.lua"
	))()
end)

if _compatAssignedGetHui then
	pcall(function() genv.gethui = _tndUiCompat.gethui end)
end
if _compatAssignedProtect then
	pcall(function() genv.protect_gui = _tndUiCompat.protect_gui end)
end
if not _uiOk then
	error("[TND Hub] UI load failed: " .. tostring(BobloUI))
end

local UI = BobloUI:CreateWindow({
	Id = "tnd-hub-steal-an-egg",
	Title = "TND Hub",
...
function H.waitFor(timeout, interval, check)
	...
	repeat
		if check and check() == true then
			passed = true
		elseif running and os.clock() < deadline then
			task.wait(gap)
		end
...
if getgenv then
	local history = getgenv().TNDHubHopHistory
	if typeof(history) ~= "table" then
		history = {}
		getgenv().TNDHubHopHistory = history
	end
...
function H.swapStealHumanoid()
	...
	if humanoid:GetAttribute("TNDStealHum") == true then
		return true
	end
	...
	replacement:SetAttribute("TNDStealHum", true)
...
function H.stealMoveTo(targetX, targetZ, keepGoing)
	...
	conn = RunService.Heartbeat:Connect(function(dt)
		...
		if (not running) or tick() >= deadline or (keepGoing and not keepGoing()) then
			failed = true
			conn:Disconnect()
			return
		end
	...
	H.track(conn)
	H.waitFor(StealConfig.MoveTimeout + 1, 0.03, function()
		return reached or failed
	end)
...
function H.runAutoReturn()
	...
	if root and PlotApi.IsWorldPositionWithinLocalPlotBounds and PlotApi.IsWorldPositionWithinLocalPlotBounds(root.Position) then
		H.waitFor(4, 0.15, function()
			return (not IsCarryingEgg) or (not H.isOn("AutoReturn"))
		end)
	end
	return true
end
...
function H.holdUid(uid)
	...
	return H.waitFor(1, 0.05, function()
...
function H.tryTeleportTo(serverId)
	...
	local failed = H.waitFor(20, 0.25, function()
		return LastTeleportFailure ~= nil or (not running)
	end)
...
function H.sendWebhookEmbed(embed, includePing)
	...
	local payload = { username = "TND Hub", embeds = { embed } }
	if includePing then
...
H.sendSummary = function()
	...
	if sent then
		...
		SummaryRebirths = 0
		H.clearTable(SpawnedEggLog)
		H.clearTable(ObtainedEggLog)
	end
	return sent
end
...
function H.destroyRenderOverlay()
	...
	H.clearTable(RenderOverlayLabels)
end
...
local function buildUI()
	...
	StealSec:AddParagraph({
		Title = "Steal Speed",
		Content = "Locked at 700. This value cannot be changed.",
		Icon = "lock",
	})
	StealSec:AddSlider({
		Id = "StealBigEggScale",
	...
	AboutSec:AddParagraph({ Title = "Script Dev", Content = "TND", Icon = "user" })
	AboutSec:AddParagraph({ Title = "UI Library", Content = "TND Hub", Icon = "layout-dashboard" })
	AboutSec:AddParagraph({ Title = "Library Dev", Content = "TND Hub", Icon = "globe" })
	...
	function H.unload()
		...
		genv.__TND_HUB_RUNNING = nil
		genv.__TND_HUB_SHUTDOWN = nil
	end

	genv.__TND_HUB_SHUTDOWN = H.unload
...
-- ===== Connections / workers (no background while-loops) =====
pcall(function()
...
H.track(RunService.RenderStepped:Connect(function(dt)
	if not running or not H.isOn("Fly") then
		return
	end
	local root = H.getRoot()
	local humanoid = H.getHumanoid()
	local cam = Workspace.CurrentCamera
	if not root or not humanoid or not cam then
		return
	end
...
local function schedulerDue(name, interval)
	local now = os.clock()
	if now < (SchedulerAt[name] or 0) then
		return false
	end
...
local function stabilizeStealRoot()
	if not H.stealingEnabled() then
		return
	end
	local root = H.getRoot()
	local humanoid = H.getHumanoid()
	if not root or not humanoid or humanoid:GetAttribute("TNDStealHum") ~= true then
		return
	end
	local laneY = H.getLaneY()
...
H.track(RunService.Heartbeat:Connect(function()
	...
	if schedulerDue("dashboard", 2) then
		...
		if noclipOn ~= LastNoClipOn then
			LastNoClipOn = noclipOn
			setNoClip(noclipOn)
		end
...
H.notify("TND Hub", "Ready", "Success", 4)
